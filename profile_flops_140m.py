...
def main():
    ...
    lines.append("")
    lines.append(
        "- Variants with NSA's compress + sliding-window branches add O(S^2) "
        "compute on top of the 2P forward base."
    )
    lines.append(
        "- ASA mode drops the compress branch → FLOPs/token should be lower than "
    ...
    lines.append(
        "- Block-fusion variants (v4-*) have a separate full-attention contextual "
