...
2.10.0+cu128. Pinned deps in `requirements.txt`. `data/` is generated by step 1
below (`train_tokens.npy` = wikitext-103, 119.1M tokens; `valid_tokens.npy` =
250k) - it is not shipped in this release.
...
# repeat per variant/lr/seed; combined into results/phase2_combined_140m_2026-05-12.md
# (dense/hymba/hymba-with-nsa/v4-gdn-nsa/v4-rwkv7-nsa/hymba-with-nsa-gated)
```

## 4. 350M proxy runs

**hymba-with-nsa (winner, PPL 87.38; run below reproduces `checkpoints_p4_350m/` locally, checkpoint not shipped):**
