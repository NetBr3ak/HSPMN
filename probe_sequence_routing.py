...
def main():
    ...
    layers = [l for _, l in gated_layers(model)] or list(model.layers)
    def ablate_all():
        return [l.nsa.register_forward_hook(
            lambda m, i, o: o._replace(out=torch.zeros_like(o.out))) for l in model.layers
            if getattr(l, "nsa", None) is not None]
    ...
    with torch.no_grad():
        for _ in range(nb):
            starts = rng.integers(0, len(valid_tok) - args.seq_len - 1, size=(args.batch,))
    ...
    seq_delta = np.array(seq_delta); seq_meanpos = np.array(seq_meanpos)
    ...
    p = float(pos.mean()); thr = binary_entropy_bits(p) - 0.5

    print(f"\nSequence-level NSA benefit, {len(seq_delta)} sequences, variant={args.variant}")
    print(f"  Δ_seq (total CE saved by NSA): mean={seq_delta.mean():.3f} std={seq_delta.std():.3f} "
          f"min={seq_delta.min():.2f} max={seq_delta.max():.2f}")
    print(f"  fraction of sequences where NSA net-helps: {p:.3f}")
    print(f"  per-token benefit variance: between-seq={between_var:.3e} within-seq={within_var_mean:.3e} "
          f"-> between fraction = {frac_between:.3f}")
    print(f"  predict sign(Δ_seq) from seq-mean embedding (held-out): acc={acc:.3f} "
          f"I(pred;sign)={mi:.4f} bits | threshold={thr:.3f}")
    verdict = ("SEQUENCE-LEVEL SIGNAL EXISTS -> routing whole sequences/segments to "
               "attention vs SSM has headroom; viable breakthrough direction."
               if (frac_between > 0.15 and mi > thr) else
               "WEAK sequence-level signal too -> on homogeneous text the contextual "
    ...
    out = Path(f"/opt/docker/LLM/HSPMN/results/sequence_routing_{args.variant}_2026-06.md")
    out.write_text(
