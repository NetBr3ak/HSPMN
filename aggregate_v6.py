...
def main():
    ...
    # ---- PPL grid -----------------------------------------------------
    lines += ["## Validation PPL by variant x pi (mean +/- std over seeds)", "",
    ...
    for v in variants:
        ...
        for p in pis:
            ...
            row.append(f"{np.mean(vals):.2f} +/- {np.std(vals):.2f} (n={len(vals)})"
                       if vals else "-")
    ...
    ceilings = {}
    for p in pis:
        val = grep_float(f"{ROOT}/results/v6_ceiling_gated_pi{p}.md",
    ...
    for p in pis:
        ...
        if g and r:
            ...
            lines.append(f"- pi=0.{p}: gated {np.mean(g):.2f}+/-{np.std(g):.2f} "
    ...
    for f in sorted(glob.glob(f"{ROOT}/results/v6_split_*.md")):
        ...
        if a is not None and b is not None:
            lines.append(f"- {os.path.basename(f)[9:-3]}: answer CE {a:.3f}, "
                         f"background CE {b:.3f}")

    # ---- P4: decor ceiling -----------------------------------------------
    lines += ["", "## P4 - decorrelation effect on ceiling (pi=0.25)", ""]
    dec = grep_float(f"{ROOT}/results/v6_ceiling_decor_gated_pi25.md",
                     r"mean held-out I\(pred;s\) = ([\d.]+) bits")
    base = ceilings.get("25")
    if dec is not None and base is not None:
        lines.append(f"- gated: {base:.4f} bits, decor-gated: {dec:.4f} bits "
    ...
    for p in pis:
        for tag in ("gated", "randgate"):
            v = grep_float(f"{ROOT}/results/v6_mi_{tag}_pi{p}.md",
                           r"mean I\(g; ?s\)[^=]*= ?([\d.]+)")
