...
class TopKRouter(nn.Module):
    ...
    def __init__(self, dim: int, target_sparsity: float = 0.2, sparsity_coef: float = 0.1, entropy_coef: float = 0.01, local_window: int = 64):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> RouterOutput:
        ...
        if self.training:
            ...
            token_entropy = -(probs * (probs + 1e-10).log() + (1.0 - probs) * (1.0 - probs + 1e-10).log()).mean()
            batch_prob = probs.mean()
            batch_entropy = -(batch_prob * (batch_prob + 1e-10).log() + (1.0 - batch_prob) * (1.0 - batch_prob + 1e-10).log())
            aux_loss = (self.token_entropy_coef * token_entropy) - (self.batch_entropy_coef * batch_entropy)
        else:
...
class LinearStateSpaceStream(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, head_dim: int, mlp_ratio: int = 4):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, q_raw: torch.Tensor, k_raw: torch.Tensor, v_raw: torch.Tensor) -> torch.Tensor:
        ...
        attn_out = torch.empty(B, S, self.num_heads, self.head_dim,
        ...
        state = torch.zeros(B, self.num_heads, self.head_dim, self.head_dim,
                            device=q_raw.device, dtype=torch.float32)
        z_state = torch.zeros(B, self.num_heads, self.head_dim,
                              device=q_raw.device, dtype=torch.float32)
        ...
        for start in range(0, _S, C):
            ...
            state = state + torch.einsum('bchk, bchv -> bhkv',
                                         k_c.float(), v_c.float())
        ...
        gate_out = torch.nan_to_num(gate_out, nan=0.0).clamp(min=-1e4)  # F.silu NaN guard
...
class HSPMNBlock(nn.Module):
    ...
    def __init__(self, config: HSPMNConfig):
        ...
        self.reflexive = LinearStateSpaceStream(config.dim, config.num_heads, config.num_kv_heads, config.head_dim, config.mlp_ratio)
    ...
    def _attention_triton(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, q_indices: torch.Tensor, kv_indices_full: torch.Tensor, B: int, S: int, D: int, seq_len_offset: int) -> torch.Tensor:
        ...
        indices_expanded = q_indices.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, self.num_heads, self.head_dim)
        ...
        # Run SQSK Attention
        attn_out = sparse_query_sparse_key_attention(q_selected, k_selected, v_selected, real_q_indices, real_full_kv_indices)
    ...
    def _attention_flex(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, q_mask: torch.Tensor, kv_indices_full: torch.Tensor, B: int, S: int, D: int, S_kv_full: int, seq_len_offset: int) -> torch.Tensor:
        ...
        block_mask = create_block_mask(mask_mod, B=B, H=1, Q_LEN=S, KV_LEN=S_kv_full + self.num_sink_tokens, device=q.device)
        
        out = flex_attention(q, k, v, block_mask=block_mask, enable_gqa=(self.num_kv_heads != self.num_heads))
        return self.o_proj(out.transpose(1, 2).contiguous().view(B, S, D))

    def _attention(self, q_raw: torch.Tensor, k_raw: torch.Tensor, v_raw: torch.Tensor, router_out: RouterOutput, past_key_values: tuple = None) -> Tuple[torch.Tensor, tuple]:
        B, S, _ = q_raw.shape
        D = self.dim
        ...
        kv_idx_exp = router_out.kv_indices.unsqueeze(1).unsqueeze(-1).expand(-1, self.num_kv_heads, -1, self.head_dim)
        ...
        if past_key_values is not None:
            ...
            kv_indices_full = torch.cat([past_kv_indices, router_out.kv_indices + seq_len_offset], dim=1)
        else:
        ...
        k_sink = self.k_proj(self.norm(sink)).view(B, self.num_sink_tokens, self.num_kv_heads, self.head_dim).transpose(1, 2)
        v_sink = self.v_proj(self.norm(sink)).view(B, self.num_sink_tokens, self.num_kv_heads, self.head_dim).transpose(1, 2)
        ...
        if HAS_TRITON_KERNELS and router_out.indices is not None and not self.training and q.is_cuda:
            attn_out = self._attention_triton(q, k_full, v_full, router_out.indices, kv_indices_full, B, S, D, seq_len_offset)
            return attn_out, new_past_key_values

        # Fallback to FlexAttention
        attn_out = self._attention_flex(q, k_full, v_full, router_out.mask, kv_indices_full, B, S, D, S_kv_full, seq_len_offset)
        return attn_out, new_past_key_values

    def forward(self, x: torch.Tensor, past_key_values: tuple = None) -> Tuple[torch.Tensor, torch.Tensor, tuple]:
        ...
        attn_out, new_past_key_values = self._attention(q_raw, k_raw, v_raw, router_out, past_key_values)
