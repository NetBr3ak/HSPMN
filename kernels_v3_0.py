...
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_M': 128, 'BLOCK_N': 64, 'PRE_LOAD_V': False}, num_stages=1, num_warps=8),
...
def _sqsk_fwd_kernel(
    ...
    stride_iqb, stride_iqm,
    stride_ikb, stride_ikn,
    sm_scale,
    Z, H, H_KV, N_ACTIVE_Q, N_ACTIVE_K,
    ...
    q_real_pos = tl.load(Q_Idx_ptr + offs_m * stride_iqm, mask=offs_m < N_ACTIVE_Q, other=-1)
    ...
    for start_n in range(0, N_ACTIVE_K, BLOCK_N):
        ...
        k_real_pos = tl.load(K_Idx_ptr + (start_n + offs_n) * stride_ikn, mask=(start_n + offs_n) < N_ACTIVE_K, other=float('inf'))
        ...
        if tl.min(k_real_pos, 0) <= max_q_pos:
            ...
            v_order: tl.constexpr = (0, 1) if V.dtype.element_ty == getattr(tl, 'float8e5', None) else (1, 0)
...
def sparse_query_sparse_key_attention(q, k, v, q_indices, k_indices):
    ...
    _sqsk_fwd_kernel[grid](
        ...
        k_indices.stride(0), k_indices.stride(1),
        sm_scale,
        q.shape[0], q.shape[2], k.shape[2], q.shape[1], k.shape[1],
        HEAD_DIM=HEAD_DIM,
    )
    return o
