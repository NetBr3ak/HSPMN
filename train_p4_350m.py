...
def parse_args():
    ...
    p.add_argument("--kill_clock_h", type=float, default=12.0,
...
def main():
    ...
    logger.info(f"train tokens: {len(train_tok):,}, valid tokens: {len(valid_tok):,}, vocab={vocab}")
    ...
    cfg = TrainConfig(
        ...
        steps=args.steps, lr=args.lr, warmup_steps=args.warmup_steps,
        weight_decay=args.weight_decay,
        eval_every=args.eval_every, eval_iters=args.eval_iters,
        log_every=args.log_every,
        nsa_window=args.nsa_window, seed=args.seed,
    ...
    for step in range(start_step, cfg.steps + 1):
        ...
        if wall > KILL_S:
            logger.warning(f"Kill clock {args.kill_clock_h}h reached at step {step}; saving and exiting.")
            break
        ...
        if step % cfg.log_every == 0:
            ...
            logger.info(f"step {step:5d}/{cfg.steps}  CE={mean_loss:.4f}  lr={cur_lr:.5f}  "
                        f"tok/s={tps:,.0f}  vram={vram_gb:.1f}GB  wall={elapsed/3600:.2f}h")
            with open(log_path, "a") as f:
                f.write(f"{step},{mean_loss:.4f},,{cur_lr:.6f},{tps:.0f},{vram_gb:.2f},{elapsed:.0f}\n")

        if step % cfg.eval_every == 0 or step == cfg.steps:
            v_ce = run_validation(model, valid_tok, cfg, device)
            logger.info(f"  → valid CE={v_ce:.4f}  ppl={math.exp(v_ce):.2f}")
            with open(log_path, "a") as f:
                f.write(f"{step},,{v_ce:.4f},{cur_lr:.6f},,,{(time.time()-t_start):.0f}\n")

        if step % args.ckpt_every == 0:
            torch.save({"model": model.state_dict(), "opt": opt.state_dict(),
                        "step": step, "config": asdict(cfg)}, state_path)
    ...
    torch.save({"model": model.state_dict(), "opt": opt.state_dict(),
                "step": min(step, cfg.steps), "config": asdict(cfg),
                "final_valid_ce": v_ce, "final_valid_ppl": math.exp(v_ce),
                "n_params": n_params,
                "tokens_seen": tokens_seen}, final_path)
    logger.info(f"Final valid CE={v_ce:.4f}  ppl={math.exp(v_ce):.2f}  ({n_params/1e6:.2f}M params)")
