...
def test_pcgate_no_params():
    ...
    assert g.min() >= 0 and g.max() <= 1, (
        f"pcgate values out of [0,1]: {g.min()} {g.max()}"
    )
...
def test_asa_alternation():
    ...
    out["loss"].backward()
    print(
        f"[5/5] asa: alternation between layers; "
...
def test_select_from_compress_zero_param_added():
    ...
    assert n_b == n_s, (
        f"select variant should be parameter-free; baseline {n_b}, select {n_s}"
    )
    ...
    print(
        "[bonus2] select-from-compress: zero added params, gradient flows through all 3 branches"
    )
