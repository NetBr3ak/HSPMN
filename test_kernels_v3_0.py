...
class TestTritonKernels(unittest.TestCase):
    ...
    def test_sqsk_attention_correctness(self):
        ...
        q_indices = torch.stack(
            [torch.randperm(S, device=self.device)[:K_active_q] for _ in range(B)]
        )
        q_indices = torch.sort(q_indices, dim=1)[0]
        k_indices = torch.stack(
            [torch.randperm(S, device=self.device)[:K_active_k] for _ in range(B)]
        )
        ...
        causal = k_indices.unsqueeze(1).unsqueeze(2) <= q_indices.unsqueeze(
            1
        ).unsqueeze(-1)
        ...
        self.assertTrue(
            torch.allclose(out_triton, out_ref, atol=1e-2, rtol=1e-2),
    ...
    def test_sqsk_handles_unaligned_q_count(self):
        ...
        self.assertFalse(
            torch.isnan(out_triton).any(), "Kernel produced NaN on unaligned tail block"
        )
