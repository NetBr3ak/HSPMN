...
def fit_mlp(Xtr, ytr, Xva, yva, hidden=64, epochs=400, lr=3e-3, wd=1e-3, device="cpu"):
    ...
    net = nn.Sequential(nn.Linear(Xtr.size(1), hidden), nn.GELU(),
...
def collect(model, valid_tok, n_batches, batch, seq, device):
    ...
    def pre_h(i, l):
        def f(mod, inp): capH[i] = inp[0].detach()
        return l.attn_gate_proj.register_forward_pre_hook(f)

    def post_c(i, l):
        def f(mod, inp, out): capC[i] = out.out.detach()  # NSAOutput.out [B,nH,S,hd]
        return l.nsa.register_forward_hook(f)

    def post_r(i, l):
        def f(mod, inp, out): capR[i] = out.detach()       # ssm out [B,S,ssm_dim]
        return l.ssm.register_forward_hook(f)

    handles = []
    for i, l in layers:
        if getattr(l, "attn_gate_proj", None) is None:
            continue
        handles.append(pre_h(i, l)); handles.append(post_c(i, l))
        if getattr(l, "ssm", None) is not None:
            handles.append(post_r(i, l))
    if not handles:
        return None

    rng = np.random.default_rng(0)
    starts = [rng.integers(0, len(valid_tok) - seq - 1, size=(batch,)) for _ in range(n_batches)]
    Fs = {i: [] for i, _ in layers}; Ss = {i: [] for i, _ in layers}
    with torch.no_grad():
        for s_off in starts:
            x = np.stack([valid_tok[a:a + seq] for a in s_off])
            x_t = torch.from_numpy(x).long().to(device)
            capH.clear(); capC.clear(); capR.clear()
            ...
            for i, l in layers:
                ...
                c = capC[i].float().norm(dim=-1).transpose(1, 2)[:, :-1, :]  # [B,S-1,nH]
                parts = [h, c]
                if i in capR:
                    r = capR[i].float()
                    nh = l.n_ssm_heads
                    r = r.view(B, r.size(1), nh, -1).norm(dim=-1)[:, :-1, :]  # [B,S-1,nH_ssm]
                    parts.append(r)
                    cmean = c.mean(-1, keepdim=True); rmean = r.mean(-1, keepdim=True)
                    parts.append(cmean - rmean)
                feat[i] = torch.cat([p.reshape(-1, p.size(-1)) for p in parts], dim=-1).cpu()
            for i, l in layers:
                if i not in feat:
                    continue
                ...
                Fs[i].append(feat[i]); Ss[i].append(s)
    for h in handles:
        h.remove()
...
def main():
    ...
    for i in sorted(data):
        Fz, S = data[i]
        n = Fz.size(0); g = torch.Generator().manual_seed(args.split_seed)
        perm = torch.randperm(n, generator=g)
        Fz, S = Fz[perm], S[perm]; ntr = int(n * 0.7)
        pva = fit_mlp(Fz[:ntr], S[:ntr].float(), Fz[ntr:], S[ntr:].float(), hidden=args.hidden, device=pdev)
        yva = S[ntr:].numpy()
    ...
    print(f"{'layer':>5} {'p(s=1)':>7} {'featdim':>7} {'val_acc':>8} {'I(pred;s) bits':>14}")
    ...
    verdict = ("ABOVE THRESHOLD -> a rich nonlinear gate could resist absorption; "
               "BUILD IT (breakthrough)." if mi_avg > thr else
               "BELOW THRESHOLD even for a nonlinear gate on both streams -> the "
    ...
    lines = [f"# Gate-ceiling probe #2 (MLP + stream features) - {args.variant}", "",
             f"Checkpoint `{args.ckpt}`.", "",
             "| layer | p(s=1) | feat dim | val_acc | I(pred;s) bits |",
             "|---|---|---|---|---|"]
    for i, p, fd, acc, mi in rows:
        lines.append(f"| {i} | {p:.3f} | {fd} | {acc:.3f} | {mi:.4f} |")
    lines += ["", f"**mean held-out I(pred;s) = {mi_avg:.4f} bits** vs threshold "
              f"{thr:.3f} bits.", "", f"**VERDICT:** {verdict}"]
    out.write_text("\n".join(lines))
    print(f"Wrote {out}")
