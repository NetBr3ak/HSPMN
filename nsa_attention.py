...
def _compress_branch(q, k, v, block_size: int, stride: int, scale: float,
    ...
    scores_masked = torch.where(row_visible, scores_masked, torch.zeros_like(scores_masked))
...
def _derive_select_indices_from_compress(scores, starts, block_size, n_select_blocks,
                                         pool_over_heads_and_queries=True):
    ...
    if pool_over_heads_and_queries:
        ...
        token_idx = (block_starts.unsqueeze(-1) +            # [B, K_block, block_size]
                     offsets.unsqueeze(0).unsqueeze(0)).clamp(max=S - 1)
...
class NSAAttention(nn.Module):
    ...
    def __init__(self, num_heads: int, head_dim: int,
        ...
        self.asa_branch = ("window" if (layer_idx % 2 == 0) else "select") \
            if mode == "asa" else None
        ...
        self.gate_proj = nn.Linear(num_heads * head_dim, n_branches * num_heads, bias=True)
    ...
    def forward(self, q, k, v, select_indices: Optional[torch.Tensor] = None) -> NSAOutput:
        B, H, S, D = q.shape
        scale = 1.0 / (D ** 0.5)
        ...
        if self.mode == "asa":
            ...
            if self.asa_branch == "window":
                ...
                out = g * out_w
                return NSAOutput(out=out, gate_compress=zeros,
                                 gate_select=zeros,
        ...
        if self.select_from_compress and select_indices is None:
            ...
            out_c = _compress_branch(q, k, v, self.compress_block_size,
                                     self.compress_stride, scale)
            out_s = _select_branch(q, k, v, select_indices, scale)
        out_w = _sliding_window_attention(q, k, v, self.window_size, scale)
