...
class HSPMNBlockV4(nn.Module):
    ...
    def __init__(
        ...
        target = float(
            router_target_sparsity
            if router_target_sparsity is not None
            else config.sparsity_k
        )
        ...
        if reflexive == "elu1":
            ...
            raise ValueError(
                f"reflexive must be 'elu1'|'gdn'|'mamba3'|'rwkv7', got {reflexive!r}"
            )
    ...
    def _attention_triton(
        self, q, k, v, q_indices, kv_indices_full, B, S, D, seq_len_offset
    ):
        q_t = q.transpose(1, 2)
        k_t = k.transpose(1, 2)
        v_t = v.transpose(1, 2)
    ...
    def _attention_flex(
        self, q, k, v, q_mask, kv_indices_full, B, S, D, S_kv_full, seq_len_offset
    ):
    ...
    def _contextual(self, x_norm, router_out, past_key_values):
        ...
        if self.attention_kind in ("nsa", "asa"):
            ...
            return attn_out, (
                k,
                v,
                torch.arange(S, device=q.device).unsqueeze(0).expand(B, -1),
            )
