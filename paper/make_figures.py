...
def fig_scaling():
    scales = ["140M", "350M"]
    hymba = np.array([139.14, 85.32]); hymba_sd = np.array([8.43, 2.64])
    dense = np.array([144.01, 83.75]); dense_sd = np.array([5.64, 2.73])
...
def fig_gate_null():
    ...
    x = np.arange(4)
    ax.bar(x, means, yerr=sds, capsize=5, color=cols, alpha=0.85,
    ...
    ax.set_ylim(75, 92)
    ax.set_title("Learned gate ties random gate at 350M\n"
...
def fig_mi_threshold():
    groups = ["140M", "350M"]
    gated = [0.0059, 0.0040]; gated_sd = [0.0004, 0.0003]
    rand = [0.0029, 0.0021]; rand_sd = [0.0006, 0.0]
    thr = 0.72
    fig, ax = plt.subplots(figsize=(5.2, 3.3))
    x = np.arange(2); w = 0.36
    ax.bar(x - w / 2, gated, w, yerr=gated_sd, capsize=4, color=C_GATED,
           label="learned gate", alpha=0.85)
    ax.bar(x + w / 2, rand, w, yerr=rand_sd, capsize=4, color=C_RAND,
           label="random gate (A–M)", alpha=0.85)
    ax.axhline(thr, color="black", ls="--", lw=1.2)
    ax.annotate(f"full-benefit threshold ≈ {thr} bits\n(½+H(s)−log2 nats)", (1.25, thr),
...
def fig_gate_ceiling():
    ...
    lin = [0.0146, 0.0180, 0.0107, 0.0231, 0.0234, 0.0206, 0.0307, 0.0308,
           0.0553, 0.0271, 0.0428, 0.0810]
    mlp = [0.0144, 0.0124, 0.0174, 0.0203, 0.0259, 0.0357, 0.0377, 0.0417,
           0.0473, 0.0481, 0.0395, 0.0686]
    thr = 0.72
    fig, ax = plt.subplots(figsize=(5.4, 3.3))
    x = np.arange(12); w = 0.4
    ax.bar(x - w / 2, lin, w, color=C_HEAD, alpha=0.85, label="linear probe on $h_t$")
    ax.bar(x + w / 2, mlp, w, color=C_GATED, alpha=0.85,
           label="MLP on $[h_t,\\|C\\|,\\|R\\|]$")
    ax.axhline(thr, color="black", ls="--", lw=1.2)
    ax.annotate(f"full-benefit threshold ≈ {thr} bits", (5.5, thr),
                xytext=(0, -3), textcoords="offset points", ha="center",
                va="top", fontsize=9)
    ax.annotate("ceiling ≈ 20× below threshold", (5.5, 0.10), ha="center",
                fontsize=9, color="#b91c1c", fontweight="bold")
    ax.set_ylim(0, 0.55)
    ax.set_xticks(x); ax.set_xticklabels(layers)
    ax.set_xlabel("layer"); ax.set_ylabel("held-out I(pred; s)  [bits]")
    ax.set_title("No gate can reach the full-benefit threshold: the NSA-helps signal\n"
...
def fig_v6_phase():
    ...
    ppl = {  # variant: (means, stds) over seeds {42,1337,2026}
        "gate-less": ([111.90, 109.63, 103.15, 88.73], [3.56, 1.64, 3.13, 2.94], C_HEAD, "o-"),
        "PC gate": ([114.71, 110.59, 101.09, 87.26], [4.68, 0.98, 1.78, 1.04], "#9333ea", "s-"),
        "learned gate": ([115.99, 110.31, 104.13, 92.61], [2.53, 2.32, 3.05, 2.47], C_GATED, "^-"),
        "random gate": ([115.60, 111.75, 105.04, 90.69], [3.46, 1.67, 0.96, 4.40], C_RAND, "v-"),
    }
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(9.6, 3.4))

    ax1.plot(pis, ceiling, "o-", color=C_HEAD, lw=2, ms=7,
             label="probe ceiling $I_{\\max}(\\pi)$")
    ax1.plot(pis, mi_gated, "s--", color=C_GATED, lw=1.5, ms=5,
             label="learned-gate $I(g;s)$")
    ax1.plot(pis, mi_rand, "v--", color=C_RAND, lw=1.5, ms=5,
             label="random-gate $I(g;s)$")
    ax1.axhline(thr, color="black", ls="--", lw=1.2)
    ax1.annotate(f"full-benefit threshold = {thr} bits", (0.25, thr),
                 xytext=(0, -4), textcoords="offset points",
                 ha="center", va="top", fontsize=8.5)
    ax1.set_yscale("log"); ax1.set_ylim(2e-3, 1.5)
    ax1.set_xlabel("recall fraction $\\pi$")
    ax1.set_ylabel("information  [bits, log]")
    ax1.set_title("P1: the ceiling rises 7.9$\\times$ with $\\pi$,\n"
    ...
    for name, (m, s, c, style) in ppl.items():
        ...
        ax2.errorbar(pis, m, yerr=s, fmt=style, color=c, lw=1.6, ms=5,
                     capsize=3, label=name, alpha=0.9)
    ax2.set_xlabel("recall fraction $\\pi$")
    ax2.set_ylabel("valid PPL (3 seeds)")
    ax2.set_title("P2: learned ties random at every $\\pi$\n"
