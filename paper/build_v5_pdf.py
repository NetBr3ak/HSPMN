...
def styles():
    ...
    s.add(
        ParagraphStyle(
            name="Sub", parent=s["Heading3"], fontSize=11, spaceBefore=8, spaceAfter=3
        )
    )
    s.add(
        ParagraphStyle(
...
def build():
    ...
    st.append(
        Paragraph(
            ...
            "the LaTeX source)</i>",
            ParagraphStyle(
                "a", parent=s["Body"], fontSize=9.5, alignment=TA_CENTER, spaceAfter=14
            ),
        )
    )
    ...
    st.append(PageBreak())
    st.append(
        Paragraph("Table 1: 140M results (3 seeds, best LR per variant)", s["Sub"])
    )
    st.append(
        table(
            [
                ...
                [
                    "v4-gdn-nsa (block fusion + router)",
                    "166.23 ± 15.12",
                    "+15.4%",
                    "35,578",
                ],
    ...
    st.append(Paragraph("Table 2: 350M results (3 seeds each, 983M tokens)", s["Sub"]))
    st.append(
        table(
            [
    ...
    st.append(
        Paragraph(
            ...
            "arrived; seed luck, told in full in the paper.",
            s["Cap"],
        )
    )
    st.append(Spacer(1, 8))
    ...
    st.append(
        Paragraph(
            ...
            "token-local gate at 24% of ideal.",
            s["Body"],
        )
    )
    ...
    fig(
        "fig_scaling_advantage.pdf",
        4.6 * inch,
        "Fig 1: Head fusion's PPL advantage decays from -3.4% (140M) to a tie at 350M.",
        s,
        st,
    )
    fig(
        "fig_gate_null_350m.pdf",
        4.6 * inch,
        "Fig 2: Learned vs random gate at 350M: means tie; learned is the "
        "higher-variance way to get nothing.",
        s,
        st,
    )
    fig(
        "fig_mi_threshold.pdf",
        4.6 * inch,
        "Fig 3: Gate mutual information vs the full-benefit threshold (log "
        "scale): two decades short at both scales.",
        s,
        st,
    )
    fig(
        "fig_gate_ceiling.pdf",
        4.6 * inch,
        "Fig 4: Optimal-probe ceiling per layer: ~0.034 bits, 21x below threshold.",
        s,
        st,
    )
    fig(
    ...
    fig(
        "router_active_fraction_heatmap.pdf",
        4.6 * inch,
        "Fig 6: Router modal collapse in block fusion: per-layer "
        "all-or-nothing despite on-target batch averages.",
        s,
        st,
    )
